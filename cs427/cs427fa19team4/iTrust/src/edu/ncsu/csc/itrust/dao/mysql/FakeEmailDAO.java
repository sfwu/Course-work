...
public class FakeEmailDAO {
	...
	public List<Email> getReminderByDoctor(String doc) throws DBException {
		Connection conn = null;
		PreparedStatement ps = null;
		try {
			conn = factory.getConnection();
			ps = conn.prepareStatement("SELECT * FROM fakeemail WHERE Instr(Body,?)>0 AND FromAddr LIKE ? ORDER BY AddedDate DESC");
			ps.setString(1, doc);
			ps.setString(2, "System Reminder");
			ResultSet rs = ps.executeQuery();
			List<Email> loadlist = emailBeanLoader.loadList(rs);
			rs.close();
			ps.close();
