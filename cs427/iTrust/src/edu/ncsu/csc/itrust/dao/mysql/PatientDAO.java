...
public class PatientDAO {
	...
	public void editPatient(PatientBean p, long hcpid) throws DBException {
		...
		try {
			...
			ps = conn.prepareStatement("UPDATE patients SET firstName=?,lastName=?,email=?,"
					...
					+ "AlternateName=?, DateOfDeactivation=?, preregistered=?, activated=? WHERE MID=?");
	...
	public long setPatient(PatientBean p) throws DBException {
		Connection conn = null;
		PreparedStatement ps = null;
		try {
			conn = factory.getConnection();
			ps = conn.prepareStatement("SELECT MAX(MID) + 1 as MID FROM Patients");
			ResultSet rs = ps.executeQuery();
			long mid = -1;
			if (rs.next()) {
				mid = rs.getLong("MID");
			...
			ps = conn.prepareStatement("INSERT INTO patients SET firstName=?,lastName=?,email=?,"
					+ "address1=?,address2=?,city=?,state=?,zip=?,phone=?,"
					+ "eName=?,ePhone=?,iCName=?,iCAddress1=?,iCAddress2=?,iCCity=?,"
					+ "ICState=?,iCZip=?,iCPhone=?,iCID=?,DateOfBirth=?,"
					+ "DateOfDeath=?,CauseOfDeath=?,MotherMID=?,FatherMID=?,"
					+ "BloodType=?,Ethnicity=?,Gender=?,TopicalNotes=?, CreditCardType=?, CreditCardNumber=?, "
					+ "DirectionsToHome=?, Religion=?, Language=?, SpiritualPractices=?, "
					+ "AlternateName=?, DateOfDeactivation=?, MID=?, preregistered=?, activated=?");

			patientLoader.loadParameters(ps, p, mid);
			ps.executeUpdate();
	 ...
	 * @throws DBException
	 */
	public void removeAllRepresenter(long representerMID) throws DBException {
		Connection conn = null;
		PreparedStatement ps = null;
		try {
			conn = factory.getConnection();
	...
	public List<PatientBean> getRenewalNeedsPatients(long hcpMID) throws DBException {
		...
		try {
			conn = factory.getConnection();
				...
				"GROUP BY final.lastname, final.firstname, final.MID HAVING COUNT(*) = 2 " +
    ...
    public List<PatientBean> getPreregistered() throws DBException{
		Connection conn = null;
		PreparedStatement ps = null;
		try {
			conn = factory.getConnection();
			ps = conn.prepareStatement("SELECT * FROM patients "
					+ "WHERE preregistered = 1 OR activated = 1");
			ResultSet results = ps.executeQuery();
			List<PatientBean> loadlist = patientLoader.loadList(results);
			results.close();
			ps.close();
			return loadlist;
		} catch (SQLException e) {
	...
	public void setPatientHCP(long m, long h) throws DBException{
		Connection conn = null;
		PreparedStatement ps = null;
		try {
			conn = factory.getConnection();
	...
	public void setPreregisteredActivated(long i) throws DBException {
		Connection conn = null;
		PreparedStatement ps = null;
		try {
			conn = factory.getConnection();
			ps = conn.prepareStatement("UPDATE patients SET preregistered= 0, activated = 1 WHERE MID=?");
			ps.setLong(1, i);
			ps.executeUpdate();
			ps.close();
		} catch (SQLException e) {
	...
	public void delPatientHCP(long l) throws DBException{
		Connection conn = null;
		PreparedStatement ps = null;
		try {
			conn = factory.getConnection();
			ps = conn.prepareStatement("DELETE FROM declaredhcp WHERE PatientID = ?");
			ps.setLong(1, l);
			ps.executeUpdate();
			ps.close();
		} catch (SQLException e) {
	...
	public void setPreregisteredDeactivated(long l) throws DBException{
		Connection conn = null;
		PreparedStatement ps = null;
		try {
			conn = factory.getConnection();
			ps = conn.prepareStatement("UPDATE patients SET preregistered= 0, activated = 0 WHERE MID=?");
			ps.setLong(1, l);
			ps.executeUpdate();
			ps.close();
		} catch (SQLException e) {
