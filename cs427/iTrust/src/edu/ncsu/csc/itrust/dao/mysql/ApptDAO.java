...
public class ApptDAO {
	...
	public List<ApptBean> getAllAppointmentsWithin(final long mid, final int n) throws SQLException, DBException {
		Connection conn = null;
		PreparedStatement pstring = null;
		try{
			conn = factory.getConnection();

			pstring = conn.prepareStatement("SELECT * FROM appointment WHERE doctor_id=? AND sched_date > NOW() AND sched_date < ? ORDER BY sched_date;");
			...
			pstring.setLong(1, mid);
			pstring.setString(2, timestamp.toString());
