...
public class ViewDiagnosisStatisticsActionTest extends TestCase {
	...
	public void testGetCauseOfDeath() throws Exception {
		try {
			DiagnosisStatisticsBean dsBean = action.getCauseOfDeath("11/02/2000", "11/20/2019", "Male");
			assertTrue(dsBean.getTopOneCauseOfDeath().equals("250.10 (1)"));
			assertTrue(dsBean.getTopTwoCauseOfDeath().equals("NONE"));
			DiagnosisStatisticsBean dsBean2 = action.getCauseOfDeath("11/02/2000", "11/20/2019", "Female");
			assertTrue(dsBean2.getTopOneCauseOfDeath().equals("NONE"));
			assertTrue(dsBean2.getTopTwoCauseOfDeath().equals("NONE"));
			DiagnosisStatisticsBean dsBean3 = action.getCauseOfDeath("11/02/2000", "11/20/2019", "");
			assertTrue(dsBean3.getTopOneCauseOfDeath().equals("250.10 (1)"));
			assertTrue(dsBean3.getTopTwoCauseOfDeath().equals("NONE"));
		} catch (FormValidationException e) {
			fail("FormValidationException");
		} catch (DBException d) {
			fail("DBException thrown");
	...
	public void testgetCauseOfDeathInvalidDate() throws Exception {
		try {
			action.getCauseOfDeath("06-28/2011", "09/28/2011", "");
			fail("Should have failed but didn't");
		} catch (FormValidationException e) {
			assertEquals(1, e.getErrorList().size());
			assertEquals("Enter dates in MM/dd/yyyy", e.getErrorList().get(0));
	...
	public void testgetCauseOfDeathReversedDates() throws Exception {
		try {
			action.getCauseOfDeath("09/28/2011", "06/28/2011", "");
			fail("Should have failed but didn't");
		} catch (FormValidationException e) {
			assertEquals(1, e.getErrorList().size());
			assertEquals("Start date must be before end date!", e.getErrorList().get(0));
