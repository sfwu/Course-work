...
public class PatientLoader implements BeanLoader<PatientBean> {
	...
	private void loadCommon(ResultSet rs, PatientBean p) throws SQLException{
		...
		p.setPreregistered(rs.getBoolean("preregistered"));
		p.setActivated(rs.getBoolean("activated"));
	}
	...
	public PreparedStatement loadParameters(PreparedStatement ps, PatientBean p){
		...
		try {
			PreparedStatement x = loadParameters(ps, p, -1);
			return x;
		}
	 ...
	 * @throws SQLException
	 */
	public PreparedStatement loadParameters(PreparedStatement ps, PatientBean p, long mid) throws SQLException {
		int i = 1;
